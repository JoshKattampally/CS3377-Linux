...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
		...
		//create a random number that the client has to guess.(between 1 and 100)
		srand(time(0));
		...
		while ((n = read(connfd, buffer, sizeof(buffer))) > 0) {
			...
			guessedNum = atoi(buffer);
			if(guessedNum > randomNumber) {
				//printf("Your guess was too large, guess again. \n");
				char message[1025] = "Your guess was too large, guess again. \n";
				write(connfd, message, strlen(message)+1);
			}
			else if(guessedNum < randomNumber) {
				char message[1025] = "Your guess was too small, guess again. \n";
				write(connfd, message, strlen(message)+1);
			}
			else {
				char message[1025] = "Congrats, that was the correct number! \n";
                                write(connfd, message, strlen(message)+1);
				connfd = -1;
			}
			//printf("Random number generated = %d \n", randomNumber);
